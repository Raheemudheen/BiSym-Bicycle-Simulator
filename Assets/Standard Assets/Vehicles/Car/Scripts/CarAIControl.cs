...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        private float m_AvoidanceSteer = 0f;
        private float m_AvoidanceSteerTimeout = 0f;
        private float m_StuckTimer = 0f;
        private float m_StuckThreshold = 5f;
        ...
        {
            m_CarController = GetComponent<CarController>();
            Debug.Log("CarController component found and assigned.");
        }
        ...
        private void FixedUpdate()
        {
            if (m_Driving)
            {
                ...
                {
                    fwd = (m_TargetPos - transform.position).normalized;
                }
                ...
                if (m_TargetPos != Vector3.zero)
                {
                    Vector3 localTarget = transform.InverseTransformPoint(m_TargetPos);
                    float targetAngle = Mathf.Atan2(localTarget.x, localTarget.z) * Mathf.Rad2Deg;

                    steer = Mathf.Clamp(targetAngle * steerSensitivity, -1, 1) + m_AvoidanceSteer;
                    desiredSpeed = m_CarController.MaxSpeed * Mathf.Clamp01((90 - Mathf.Abs(targetAngle)) / 90);

                    Debug.Log($"Steer: {steer}, Desired Speed: {desiredSpeed}, Target Position: {m_TargetPos}, Current Position: {transform.position}");
                }

                if (m_AvoidanceSteerTimeout > 0)
                {
                    m_AvoidanceSteerTimeout -= Time.fixedDeltaTime;
                    if (m_AvoidanceSteerTimeout <= 0)
                    {
                        m_AvoidanceSteer = 0f;
                        Debug.Log("Avoidance steer timeout reset to: 0");
                    }
                }

                float accel = desiredSpeed * accelSensitivity;
                float brake = 0;
                ...
                //    float slowDownFactor = Mathf.Clamp01(distanceToObstacle / obstacleDetectionDistance);
                //    accel *= slowDownFactor;
                //    brake = (1 - slowDownFactor) * brakeSensitivity;

                //    obstacleDetected = true;
                //    Debug.Log($"Obstacle detected in front! Distance: {distanceToObstacle}, SlowDownFactor: {slowDownFactor}, Adjusted Accel: {accel}, Adjusted Brake: {brake}");
                //}

                //if (!obstacleDetected)
                //{
                //    float leftAvoidance = 0f;
                //    float rightAvoidance = 0f;

                //    if (Physics.Raycast(transform.position, transform.forward + transform.right * 0.5f, out hit, obstacleDetectionDistance, obstacleLayer))
                //    {
                //        float distanceToObstacle = hit.distance;
                //        float slowDownFactor = Mathf.Clamp01(distanceToObstacle / obstacleDetectionDistance);
                //        rightAvoidance = (1 - slowDownFactor) * 0.5f;
                //    }
                ...
                //        float distanceToObstacle = hit.distance;
                //        float slowDownFactor = Mathf.Clamp01(distanceToObstacle / obstacleDetectionDistance);
                //        leftAvoidance = (1 - slowDownFactor) * 0.5f;
                //    }

                //    if (leftAvoidance > 0f || rightAvoidance > 0f)
                //    {
                //        if (leftAvoidance > rightAvoidance)
                //        {
                //            m_AvoidanceSteer = -leftAvoidance;
                //            m_AvoidanceSteerTimeout = 1f;
                //            Debug.Log($"Obstacle detected on the right! Adjusting steer left by: {-leftAvoidance}");
                //        }
                //        else
                //        {
                //            m_AvoidanceSteer = rightAvoidance;
                //            m_AvoidanceSteerTimeout = 1f;
                //            Debug.Log($"Obstacle detected on the left! Adjusting steer right by: {rightAvoidance}");
                //        }
                //    }
                //}

                Debug.Log($"Steer: {steer}, Accel: {accel}, Brake: {brake}, IsReversing: {m_IsReversing}");

                if (m_IsReversing)
                {
                    m_CarController.Move(steer, -accelSensitivity, brakeSensitivity, 0);
                    Debug.Log("Reversing...");
                }
                else
                {
                    m_CarController.Move(steer, accel, brake, 0);
                }

                if ((transform.position - m_LastPosition).sqrMagnitude < 0.01f)
                {
                    ...
                    if (m_StuckTimer >= m_StuckThreshold)
                    {
                        ...
                        m_StuckTimer = 0f;
                        Debug.Log($"Stuck detected. Reversing: {m_IsReversing}");
                    }
                }
                else
                {
                ...
                if (stopWhenTargetReached && Vector3.Distance(transform.position, m_TargetPos) < reachTargetThreshold)
                {
                    m_Driving = false;
        ...
        public void SetTarget(Transform target)
        {
            ...
            Debug.Log($"Target set to: {m_TargetPos}");
        }

        public void SetAvoidanceSteer(float steer, float duration)
        {
            m_AvoidanceSteer = steer;
            m_AvoidanceSteerTimeout = duration;
            Debug.Log($"Avoidance steer set to: {steer} for duration: {duration}");
        }
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawRay(transform.position, (transform.forward + transform.right * 0.5f).normalized * obstacleDetectionDistance);
