...
namespace UnityStandardAssets.Utility
{
    public class WaypointProgressTracker : MonoBehaviour
    {
        ...
        public float avoidanceSteerFactor = 0.5f;
        ...
        private void Start()
        {
            if (circuit == null)
            {
                Debug.LogError("Waypoint circuit not assigned.");
                enabled = false;
                return;
            }
        ...
        public void Reset()
        {
            ...
            carAIControl.SetTarget(target); // Set the initial target
        }
        ...
        private void Update()
        {
            ...
            carAIControl.SetTarget(target); // Continuously update the target
        ...
        private void DetectAndAvoidObstacles()
        {
            if (carAIControl == null)
            {
                Debug.LogError("CarAIControl component not assigned.");
                return;
            }
            ...
            foreach (var direction in rayDirections)
            {
                ...
                {
                    float steer = Mathf.Sign(hit.normal.x) * avoidanceSteerFactor;
                    carAIControl.SetAvoidanceSteer(steer, avoidanceDuration);
                    obstacleDetected = true;
                    Debug.Log($"Obstacle detected! Adjusting steer by: {steer}");
                    break;
                }
            }
            ...
            if (!obstacleDetected)
            {
                carAIControl.SetAvoidanceSteer(0f, 0f);
                Debug.Log("No obstacle detected. Avoidance steer reset to: 0");
            }
        }
    }
}
